...
export default function CreativeStudioPanel({
  ...
  // page.tsx's scene/asset query (and the timeline built from it) is only
  // re-run on navigation/refresh — without this, a batch could finish
  // completely and the timeline/preview would still show nothing until the
  // user manually reloaded the tab. Fires once on the true->false edge
  // (jobsWereActiveRef), not on every poll tick while jobs are still
  // running, since router.refresh() re-runs the whole server component and
  // there's no reason to pay that cost every 4s.
  const jobsWereActiveRef = useRef(false);
  useEffect(() => {
    if (jobsActive) {
      jobsWereActiveRef.current = true;
    } else if (jobsWereActiveRef.current) {
      jobsWereActiveRef.current = false;
      router.refresh();
    }
  }, [jobsActive, router]);
