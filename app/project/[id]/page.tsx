...
// TimelineEditor; the right panel runs the actual prompt -> script ->
...
export default async function StudioPage({
  ...
  const project = await prisma.project.findUnique({
    ...
    include: {
      ...
      scenes: {
        orderBy: { order: "asc" },
        include: {
          // Only need the latest successful clip per scene for display.
          generatedVideos: {
  ...
  const dbTracks: EditorTrack[] = (
    project.timeline?.tracks ?? []
  ).map((t) => ({
    id: t.id,
    // Prisma's generated TrackType enum and our local TrackType union share
    // the same runtime string values; cast through unknown at this boundary.
          ...
          {LEFT_TOOLS.map((tool) =>
            tool === "Brand" ? (
              <BrandToolButton
                ...
                initialBrandKit={project.brandKit}
              />
            ) : (
              <button
                key={tool}
